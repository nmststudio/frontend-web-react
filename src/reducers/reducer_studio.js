...
export default function(state = INITIAL_STATE, action) {
    ...
    switch (action.type) {
        ...
        case CREATE_STUDIO: // start fetching posts and set loading = true
            return { ...state, studioList: { studios: [], error: null, loading: true } };
        ...
        case FETCH_STUDIOS:
            return { ...state, studioList: { ...state.studioList, loading: true } }
        case FETCH_STUDIOS_SUCCESS:
            return { ...state, studioList: { studios: action.payload, error: null, loading: false } }
        case FETCH_STUDIOS_FAILURE:
            error = action.payload || { message: action.payload.message }; //2nd one is network or server down errors
            return { ...state, studioList: { post: null, error: error, loading: false } }
